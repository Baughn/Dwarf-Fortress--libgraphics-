/* compat wrapper for musl, support GLIBCs *_chk functions
   from http://port70.net/~nsz/musl/compat

   gcc musl-glibc-compat-preload.c -fPIC -shared -O3 -o compat.so

   then use LD_PRELOAD=compat.so
*/
...
_Noreturn void __longjmp_chk(jmp_buf buf, int v)
{
	longjmp(buf, v);
}
