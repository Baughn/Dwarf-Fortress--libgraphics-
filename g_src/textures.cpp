...
string tileset::load_multi_pdim(const string &filename, int dimx,
			       int dimy, bool convert_magenta,
			       int *disp_x, int *disp_y) {
  SDL_Surface *raw = IMG_Load(filename.c_str());
  if (!raw) {
    return "Not found: " + filename;
  }
  load_multi_pdim(raw, dimx, dimy, convert_magenta, disp_x, disp_y);
  SDL_FreeSurface(raw);
  return "";
}

void tileset::load_multi_pdim(SDL_Surface *raw, int dimx, int dimy,
...
static void test_file(string &out, string in) {
  ifstream s(in);
  if (s.good())
    out = in;
}

string tileset::load_tileset(const string &filename, bool convert_magenta) {
  vertex_shader   = "data/art/shader.vs";
  fragment_shader = "data/art/shader.fs";
  geometry_shader = "data/art/shader.gs";
  // First, try to read this as an old-style tileset
  SDL_Surface *raw = IMG_Load(filename.c_str());
  if (raw) {    // Old-style.
    load_multi_pdim(raw, 16, 16, convert_magenta, &dispx, &dispy);
    SDL_FreeSurface(raw);
    return "";
  }
  // New-style, then?
  ifstream descriptor(filename + "/tileset.txt");
  if (!descriptor.good()) {
    clear();
    return "Neither " + filename + " nor " + filename + "/tileset.txt found.";
  }
  string title;
  getline(descriptor, title);
  cout << "Loading tileset: " << title << endl;
  string array;
  double mag;
  int arraysz = 0;
  while (descriptor.good()) {
    string line;
    ...
    if (line.size() == 0 || line[0] == '#')
      continue;
    // Okay, let's call it a normal command.
    istringstream s(line);
    string command;
    s >> command;
    if (command == "array") {
      // Close out the current array, if any
      if (arraysz) {
#ifdef DEBUG
        cout << "Closing array of " << arraysz << " tiles\n";
#endif
        arrays[array] = make_pair(arraysz, mag);
        arraysz = 0;
      }
      // Start a new array
      s >> array;
      s >> mag;
#ifdef DEBUG
      cout << "Starting tileset array " << array << " at " << tiles.size() << ", mag " << mag << endl;
#endif
    } else {
      // Add tiles to this array
      int w, h;
      s >> w >> h;
#ifdef DEBUG
      cout << "  Loading " << w << "x" << h << " page " << command << endl;
#endif
      arraysz += w * h;
      if (!dispx || !dispy) {
        string r = load_multi_pdim(filename + "/" + command, w, h, convert_magenta, &dispx, &dispy);
        if (r.size()) {
          clear();
          return r;
        }
        ...
        int dummy1, dummy2;
        string r = load_multi_pdim(filename + "/" + command, w, h, convert_magenta, &dummy1, &dummy2);
        if (r.size()) {
          clear();
          return r;
        }
  ...
  if (arraysz)
    arrays[array] = make_pair(arraysz, mag);
  // Test for custom shaders
  test_file(vertex_shader, filename + "/shader.vs");
  test_file(fragment_shader, filename + "/shader.fs");
  test_file(geometry_shader, filename + "/shader.gs");
  return "";
}

vector<pair<string, vector<SDL_Surface*> > > tileset::get_all_textures(int width, int height) {
  vector<pair<string, vector<SDL_Surface* > > > ret;
  auto tile = tiles.begin();
  for (auto it = arrays.begin(); it != arrays.end(); ++it) {
    vector<SDL_Surface*> array;
    for (int i=0; i < it->second.first; ++i, ++tile) {
...
SDL_Surface *tileset::get_texture(int tile, int w, int h) {
  return SDL_Resize(tiles[tile], w, h, false);
}
...
// void tileset::remove_uploaded_textures() {
//   if (!uploaded) return; // Nothing to do
//   glDeleteTextures(1, &gl_catalog);
//   uploaded=false;
// }

// SDL_Surface *tileset::get_texture_data(long pos) {
//   if (raws.size() > pos)
//     return raws[pos];
//   else {
...
// long tileset::clone_texture(long src) {
//   long tx;
...
// void tileset::grayscale_texture(long pos) {
...
// long tileset::add_texture(SDL_Surface *surface) {
//   long sz = raws.size();
...
// long tileset::load(const string &filename, bool convert_magenta) {
//   SDL_Surface *raw = IMG_Load(filename.c_str());
//   if (!raw) {
...
// void tileset::delete_texture(long pos) {
